      ...
      if items['data'].present?
        items['data'].each do |item|
          ...
          Turn14Product.add_t14_product(
            ...
            item_hash['prop_65'],
            item_hash['epa'],
            item_hash["part_description"],
            item_hash["category"],
            item_hash["subcategory"],
            item_hash["dimensions"].collect{|x| x["box_number"]}[0],
            item_hash["dimensions"].collect{|x| x["length"]}[0],
            item_hash["dimensions"].collect{|x| x["width"]}[0],
            item_hash["dimensions"].collect{|x| x["height"]}[0],
            item_hash["dimensions"].collect{|x| x["weight"]}[0],
            item_hash["carb_eo_number"],
            item_hash["clearance_item"],
            item_hash["thumbnail"],
            item_hash["units_per_sku"]
          )
        end
      end
