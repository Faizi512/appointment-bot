      ...
      Zip::File.open('zip_inventory') do |zip_file|
        ...
        zip_file.each do |entry|
          ...
          content.split("\n").each do |row|
            puts row.inspect
            row = row.to_s.split(",")
            if row[0] != "VendorName" && row[2] != "VendorCode"
              ...
              product.update(vendor_name: vendor_name, vcpn: vcpn, vendor_code: vendor_code, 
              part_number: part_number, manufacturer_part_no: manufacturer_part_no, 
              long_description: long_description, jobber_price: jobber_price, cost: cost, 
              ups_able: ups_able, core_charge: core_charge, case_qty: case_qty, 
              is_non_returnable: is_non_returnable, upc_code: upc_code, total_qty: total_qty, 
              kit_components: kit_components, is_kit: is_kit)
            end  
          end
        end
      end
